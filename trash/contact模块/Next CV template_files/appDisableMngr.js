(function (window, $, undefined) {
    ...
    window.mpvAppDisableManager = {
        ...
        "isDomainBlackList": function () {
            ...
            try {
                var prdct = mpvInterface.getParam("prdct"), isBlacklisted = false, blackListedGlobally, blackListedPrdct;
                blackListedGlobally = mpvBlackList.global && mpvBlackList.global.test(mpvInterface.getDomain(mpvInterface.getPageURL()));
                blackListedGlobally = blackListedGlobally && !mpvInterface.getParam("allowOnGlobalBlackList");
                blackListedPrdct = mpvBlackList[prdct] && mpvBlackList[prdct].test(mpvInterface.getDomain(mpvInterface.getPageURL()));
                isBlacklisted = blackListedGlobally || blackListedPrdct;
                if (!isBlacklisted) {

                    isBlacklisted = this.isSpecificCaseBlackList();
                }
        ...
        "getDomainBlackListStatus" : function(){

            try {
                // determine whether the domain is blocked and by which level 
                var prdct = mpvInterface.getParam("prdct");
                return {

                    "globalBlacklisted": mpvBlackList.global && mpvBlackList.global.test(mpvInterface.getDomain(mpvInterface.getPageURL())),
                    "prdctBlacklisted": mpvBlackList[prdct] && mpvBlackList[prdct].test(mpvInterface.getDomain(mpvInterface.getPageURL()))
        ...
        "isSpecificCaseBlackList" : function(){

            try {
                // add any expections that cannot blacklist per domain.
                var blacklisted = false;
                if(mpvInterface.getParam("prdct") ===  "moshempvn"){

                    blacklisted = /^https/i.test(mpvInterface.getPageURL())
        ...
        "disableApp": function (callback, numberOfAttemps) {
            ...
            try {
                ...
                $.when($.ajax({
                    ...
                    "data": { "hrdId": mpvInterface.getParam("hrdId"), "app": mpvInterface.getParam("prdct") },
                    "dataType": "jsonp",
        ...
        "enableApp": function (callback, numberOfAttemps) {

            try {
                if (!this.isAppEnabled()) {
                    var that = this;
                    numberOfAttemps = numberOfAttemps || 0;
                    this.setEnableAppFlash();

                    $.when($.ajax({

                        "url": mpvInterface.getParam("disableAppURL") + "/ActivateUser",
                        "data": { "hrdId": mpvInterface.getParam("hrdId") },
                        "dataType": "jsonp",
                        "jsonpCallback": "mpvSetServerDisableApp",
                        "cache": true
                        ...
                        try {
                            ...
                            if (numberOfAttemps >= 3 && (!response || response.status !== "OK")) {
                                ...
                                return that.enableApp(callback, numberOfAttemps);
                            }
        ...
        "isAppEnabled": function () {
            ...
            try {
                return mpvStorageMngr.getItem("mpvStatus") !== "0" && mpvStorageMngr.getItem("mpvStatus") !== 0;
            }
